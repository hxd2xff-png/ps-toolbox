### v1.7.6（诊断增强：选区/识别/应用目标全链路日志）

- 纯诊断版本，**无行为改动**：为定位「识别到未选中图层 / 应用打到其他对象」补齐证据通道
- `diag.log` 现在记录：每次选区变化的 id+来源+`targetLayers` 失败原因（`sel`）、每次识别实际读取的图层 id/名字与回填被哪道手选闸门阻挡（`fill`）、每次应用实际处理的图层 id 列表（`mix-done`）
- `targetLayerIds()` 三种静默失败模式（throw / no-key / 索引解析失败）不再无痕，原因随回包返回

### v1.7.5（中文字体必定先行 · 字重不再静默假成功 · 请求侧诊断）

- **中文字体必定落盘**（按识别规范 §10.2）：应用时先用 DOM 把整层文本设为中文字体，再按区间切英文字体——即使区间描述符被 Photoshop 拒绝，中文字体也已经写进去了；
- **修「面板报成功、字符面板却是另一个字重」**：面板请求 `Noto Sans SC / Regular` 而 PS 实际存成同族另一字重时，旧版「同族不算替换」的豁免会把失败静默吞掉。现在如实上报「字重未生效：请求 X，实际 Y」，并自动重写一次后回读校验；
- **空修复不再变成功**：自修复找不到可换区间时保留原始失败判定，绝不把真问题洗成 ok；
- **请求侧诊断**：diag.log 每次应用记录 `mix-req`（面板发出的字体配置）与 `mix-resolve`（宿主解析出的 PostScript 名），一行定位字重回退。

### v1.7.4（启动不再卡机 · 分类与 Figma 参考逐字符一致）

- **启动卡机根治**：连接即就绪，字体库拉取（800+ 家族跨桥）延迟到 2.5s 空闲后，环境探测延迟到 3s 后且改为轻量版（不再在启动帧构建字体索引 + 遍历图层树）
- **字符分类与 Figma 参考实现逐字符一致**（diff 测试 100% 通过）：
  - 自动模式：ASCII 标点（`-` `_` `(` `)` 等）归**英文字体**（拉丁字体自带这些字形）；全角/CJK 标点归中文字体
  - 空格/制表/换行：永远跟随前一字符侧，不受符号开关影响
  - 符号开关显式选择时：所有标点符号（含 ASCII）跟随开关
  - 罗马数字规则不变：Unicode Ⅰ–Ⅻ 与合规 ASCII 罗马（I..XXV）优先归中文
